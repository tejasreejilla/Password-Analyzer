...
calculateTheoreticalEntropy(password) {
    const pool = this.getCharacterSpace(password);
...
```javascript
const PATTERNS = [
    { regex: /(.)\1{2,}/, name: 'Repeated chars' },
    { regex: /(012|123|234...)/, name: 'Sequential numbers' },
    { regex: /(abc|bcd|cde...)/, name: 'Sequential letters' },
    { regex: /(qwerty|asdfgh)/, name: 'Keyboard pattern' }
];
```

**Variation Detection:**
```javascript
checkSimpleVariation(password) {
...
**Formula:**
```
Total = Length(0-25) + Complexity(0-25) + 
        Entropy(0-25) + Dictionary(0-25)
```
...
calculateCrackTime(password) {
    ...
    const attemptsPerSec = 1e9; // 1 billion/sec
